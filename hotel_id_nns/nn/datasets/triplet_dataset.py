...
class TripletDataset(H5HotelDataset):
    ...
    def __getitem__(self, index) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        while rand_index != index:
            rand_index = pos_anchors[torch.randint(high=len(pos_anchors), size=(1,)).item()]
        positive_anchor = self.preprocess(torch.as_tensor(self.imgs[rand_index]))
        
        # negative
        neg_anchors = (self.chain_ids != chain_id).nonzero()
        assert len(neg_anchors) >= 1, 'at least one negative sample'
        rand_index = neg_anchors[torch.randint(high=len(neg_anchors), size=(1,)).item()]
